  ...
  <a-layout class="userMgt">
    <a-layout-content :style="{ margin: '24px 16px', padding: '24px',background: '#fff', minHeight: '540px' }">
      <div>
        ...
        <a-table :columns="columns" :dataSource="paryList" rowKey="prayerId" :pagination="pagination">
                <span slot="action" slot-scope="text, record">
                  <a href="javascript:void(0)" class="actionbtn br" @click="editGrade(record)">编辑</a>
                  <a-popconfirm title="确定删除该条信息？" okText="确定" cancelText="取消" @confirm="delPray(record.prayerId)">
  ...
  export default {
    data () {
      return {
        ...
        columns: [
          {
            title: '标题',
            dataIndex: 'prayerTitle',
            align: 'center',
            width: 400
          },
          {
            title: '内容',
            dataIndex: 'prayerContent',
            align: 'center'
          },
          {
            title: '时间',
    ...
    methods: {
      ...
      queryPray(){
        ...
        this.$Utils
          .commonAjax({
            url: api.interfaceConfig.queryPray,
            data: params,
            type: 'POST',
            flag: true
          }).then(res => {
            if (res.success) {
              this.paryList = res.result ? res.result.list : []
      ...
      delPray(id){
        ...
        this.$Utils
          .commonAjax({
            url: api.interfaceConfig.deletePray,
            type: 'POST',
            data: params,
            flag: true
          }).then(res => {
